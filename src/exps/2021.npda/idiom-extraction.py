...
class GreedyIdiomMiner:
    ...
    def evaluation(self):
        ...
        for k, (stat_train, stat_dev) in enumerate(self.stat_by_iter):
            step_stats = self.step_evaluation(stat_train, stat_dev, self.rule_to_id)
    ...
    @classmethod
    def step_evaluation(cls, train_stat: dict, dev_stat: dict, rule_to_id: dict) -> Dict[str, Dict[str, float]]:
        stat = OrderedDict()
        # grammar count
        ...
        train_rule_stats = cls.describe(list(train_counter.values()))
        ...
        train_heights_stat = cls.describe([x['height'] for x in train_stat['trees_stat']])
        train_rule_num_stat = cls.describe([x['rule_num'] for x in train_stat['trees_stat']])
        train_distinct_rule_num_stat = cls.describe([x['distinct_rule_num'] for x in train_stat['trees_stat']])
        ...
        dev_heights_stat = cls.describe([x['height'] for x in dev_stat['trees_stat']])
        dev_rule_num_stat = cls.describe([x['rule_num'] for x in dev_stat['trees_stat']])
        dev_distinct_rule_num_stat = cls.describe([x['distinct_rule_num'] for x in dev_stat['trees_stat']])
        ...
        train_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in train_stat['trees_stat']], 'base', rule_to_id)
        dev_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in dev_stat['trees_stat']], 'base', rule_to_id)
        stat["base_train_ll_rec"] = train_ll_rec
        stat["base_dev_ll_rec"] = dev_ll_rec
        train_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in train_stat['trees_stat']], 'oov', rule_to_id)
        dev_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in dev_stat['trees_stat']], 'oov', rule_to_id)
        stat["oov_train_ll_rec"] = train_ll_rec
        stat["oov_dev_ll_rec"] = dev_ll_rec
        train_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in train_stat['trees_stat']], 'full', rule_to_id)
        dev_ll_rec = cls.dataset_stat(train_counter, [x['rule_count'] for x in dev_stat['trees_stat']], 'full', rule_to_id)
        stat["full_train_ll_rec"] = train_ll_rec
        stat["full_dev_ll_rec"] = dev_ll_rec

        # how close the two MLE estimated measures are
        kl_stats = cls.kl_divergence(train_counter, dev_stat['rule_dist'])
        stat["kl_stats"] = kl_stats
        return stat

    @classmethod
    def kl_divergence(cls, c1: Counter, c2: Counter):
        ...
        for (nt, rule_id) in common_rules:
            p = cls.rule_density(c1, rule_id, nt, 'base', total_p)
            q = cls.rule_density(c2, rule_id, nt, 'base', total_q)
            kl_p_q += p * math.log(p / q)
            kl_q_p += q * math.log(q / p)

        smooth_p_q = smooth_q_p = 0
        for (nt, rule_id) in union_rules:
            p = cls.rule_density(c1, rule_id, nt, 'oov', total_p)
            q = cls.rule_density(c2, rule_id, nt, 'oov', total_q)
    ...
    @classmethod
    def rule_density(self, counter: Counter, rule_id: int, nt: str,
    ...
    @classmethod
    def dataset_stat(cls, counter: Counter, tree_rule_counts: List[Counter],
        ...
        for tree_count in tree_rule_counts:
            ...
            for (nt, rule_id), freq in tree_count.items():
                density = cls.cond_rule_density(counter, rule_id, nt, support, complete_nt_num)
                if density == 0:
                    ll_buffer = None
                    break
    ...
    @classmethod
    def cond_rule_density(cls, counter: Counter, rule_id: int, nt: str,
                          support: Literal['base', 'oov', 'full'] = 'base',
...
def main(prefix=""):
    names = [n for n in os.listdir('.') if n.endswith('parse.pkl')]
    for name in names:
        trees = pickle.load(open(name, 'rb'))
        miner = GreedyIdiomMiner(trees[0], trees[1], name[:name.index('.pkl')], data_prefix=prefix)
        # miner = pickle.load(open('geo_cg.sqlite-parse.363.miner_state', 'rb'))
        miner.mine()
        miner.evaluation()
