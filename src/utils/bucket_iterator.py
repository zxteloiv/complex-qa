...
class BucketIterator(Iterator):
    ...
    def __next__(self):
        ...
        while data is None:
            ...
            if self.is_new_epoch:   # the batch data is empty and the last, dataset exhausted.
                raise StopIteration
            data = self._get_next_batch()
        return data
    ...
    def _get_next_batch(self):
        ...
        if self._current_bucket >= len(self._batches):
            self._current_bucket = 0
            self.epoch += 1
            # the current batch is the last of the epoch, and the is_new_epoch flag is set True
        ...
        tensor = self.batch_to_tensors(batch)
        return tensor
