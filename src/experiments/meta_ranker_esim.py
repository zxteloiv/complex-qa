...
def _atis_base():
    ...
    p.num_encoder_layer = 2
    p.num_composition_layer = 2
...
def get_aux_model(hparams, vocab):
    from experiments.build_model import get_esim
    model: torch.nn.Module = get_esim(hparams, vocab)
...
class MAMLUpdater(Updater):
    ...
    def _evaluation_updater(self, batch, model, inner_opt):
        ...
        correct_score = task_logits.log_softmax(dim=-1)[:, 1]

        return {"ranking_score": correct_score, "label": label,
