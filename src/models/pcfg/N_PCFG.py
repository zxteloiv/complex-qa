...
class NeuralPCFG(nn.Module):
    def __init__(self, args, dataset):
        ...
        # if not using this initialization, the performance would get much worser.
        self._initialize()
    ...
    def forward(self, input, evaluating=False):
        ...
        def terms():
            ...
            term_prob = self.term_mlp(term_emb).log_softmax(-1)         # (b, n, T, V)
            indices = x.unsqueeze(2).expand(b, n, self.T).unsqueeze(3)  # (b, n, T, 1)
            term_prob = torch.gather(term_prob, 3, indices).squeeze(3)  # (b, n, T)
            return term_prob
    ...
    def loss(self, input):
        ...
        result = self.pcfg.inside(rules=rules, lens=input['seq_len'])
