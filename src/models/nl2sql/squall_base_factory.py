...
from models.base_s2s.model_factory import RNNListMixin
...
class SquallBaseBuilder(RNNListMixin):
    ...
    def get_model(self):
        ...
        parser = SquallBaseParser(
            ...
            col2input=nn.Sequential(ResLayer(hid_sz, hid_sz),
                                    VDrop(dropout, on_the_fly=False)),
            span2input=nn.Sequential(ResLayer(hid_sz * 2, hid_sz),
                                     VDrop(dropout, on_the_fly=False)),
            ...
            sql_type=nn.Linear(hid_sz * 3, len(tgt_type_keys)),
            sql_keyword=nn.Linear(hid_sz * 3, vocab.get_vocab_size(p.ns_keyword)),
            sql_col_type=nn.Linear(hid_sz * 3, vocab.get_vocab_size(p.ns_coltype)),
            sql_col_copy=get_attn(p.col_copy, hid_sz * 3, hid_sz),
            sql_span_begin=get_attn(p.span_begin, hid_sz * 3, hid_sz),
            sql_span_end=get_attn(p.span_end, hid_sz * 3, hid_sz),
